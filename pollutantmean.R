pollutantmean <- function(directory, pollutant, id = 1:332) {
        ...
        polldat <- data.frame() #instantiate data frame

        for (i in 1:length(files_list)) { #fill data frame with file data
            polldat <- rbind(polldat, read.csv(files_list[i]))
        }
        
        polldat_subset <- subset(polldat, ID %in% id) #filter data frame ID col by id
